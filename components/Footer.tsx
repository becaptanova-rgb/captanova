...
const footerLinks = {
  ...
  Connect: [
    { label: "WhatsApp", href: `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "919876543210"}` },
...
const stepColors = ["var(--accent-primary)", "var(--accent-royal)", "var(--accent-cyan)", "var(--accent-royal)", "var(--accent-ink)", "var(--accent-slate)"];

export default function Footer() {
  return (
    <footer className="relative bg-white border-t border-black/[0.06] py-16 px-6 overflow-hidden">
      <div className="glow-blob w-80 h-80 bg-blue-50 top-0 left-1/4" />
        ...
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="md:col-span-1">
            <div className="mb-5">
              ...
              <p className="text-[var(--text-tertiary)] text-[8px] tracking-[2px] uppercase mt-1 font-medium">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <p className="text-[var(--text-tertiary)] text-[10px] tracking-[2px] uppercase font-bold mb-5">
                {category}
              </p>
            ...
            {["Awareness", "Clarity", "Confidence", "Communication", "Action", "Leadership"].map(
              (step, i, arr) => (
                <span key={step} className="flex items-center gap-3">
                  ...
                  {i < arr.length - 1 && <span className="text-[var(--text-tertiary)] text-base">→</span>}
                </span>
              )
            )}
          </div>
        ...
        <div className="border-t border-black/[0.06] pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-6">
            {[
              ...
              <a key={item.label} href={item.href} className="text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] text-base transition-colors">
                {item.label}
              </a>
            ))}
          </div>
