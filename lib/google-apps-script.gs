 ...
 *
 * The sheet will auto-create three tabs: "Leads", "Purchases", and "Masterclass Registrations"
 */

const SHEET_NAME_LEADS = "Leads";
const SHEET_NAME_PURCHASES = "Purchases";
const SHEET_NAME_MASTERCLASS = "Masterclass Registrations";
...
function doPost(e) {
  try {
    ...
    } else if (data.type === "masterclass") {
      // ── Handle free masterclass registration ─────────────────────────────
      let sheet = ss.getSheetByName(SHEET_NAME_MASTERCLASS);
      if (!sheet) {
        sheet = ss.insertSheet(SHEET_NAME_MASTERCLASS);
        sheet.appendRow([
          "Timestamp", "Name", "Email", "Phone", "City", "Age", "Source"
        ]);
        sheet.getRange(1, 1, 1, 7).setFontWeight("bold").setBackground("#1a1a2e")
          .setFontColor("#D4AF37");
      }
      sheet.appendRow([
        data.timestamp || new Date().toISOString(),
        ...
        data.city || "",
        data.age || "",
        data.source || "masterclass-landing",
      ]);
    } else {
      // ── Handle lead ────────────────────────────────────────────────────
